               ...
               std::vector<QString>& error_messages)
{
    ...
    try {
        ...
        {
            registered_node_names.insert(str.toStdString());
        }

        // Convert set to unordered_map
        std::unordered_map<std::string, BT::NodeType> registered_nodes;
        for(const auto& name : registered_node_names)
        {
            registered_nodes[name] = BT::NodeType(); // Initialize with default NodeType
        }
